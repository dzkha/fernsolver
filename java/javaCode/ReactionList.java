...
class ReactionList extends Panel {
    ...
    // serialNumber holds the serialIndex that uniquely identifies a
    //  reaction by giving its sequence number in the deserialization from the disk file.
    ...
    public ReactionList (){
        ...
        for(int i=0; i<nR; i++) {
            String cbString = " "+rArray[i].reacString+"  Q = "
                +rArray[i].Q+"  Class = " +rArray[i].reacIndex+"  Serial = "+serialNumber[i];
            cb[i] = new Checkbox(cbString,false);
            cb[i].setBackground(color2);
            int rc = rArray[i].reacIndex;                 // Reaction class
            if(IsoData.checkBox[rc].getState()) {   // If class selected
                this.add(cb[i]);
                if(!DataHolder.RnotActive[Z][N][i]) {
                    DataHolder.RnotActive[Z][N][i] = !StochasticElements.pruneReactions(Z, N, rArray[i]);
                }
